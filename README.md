...
- **Per account**: each queued capture remembers which account it was made under and only syncs into that account, so two accounts on one device never mix. Signing out with captures still waiting warns first; they stay on the device until that account signs back in. Captures queued before this existed go to whichever account syncs first.
...
- **Share to Loose Change** (Android / desktop Chrome, installed app only): sharing a link or text from another app saves it straight to the Inbox as a text capture — same offline queue, "Saved ✓", then Record. The manifest's `share_target` points at `/` rather than a separate route, because Record's shell is the one the service worker keeps for offline starts. Signed out, sign-in carries the shared text through and saves it afterwards.
- **Home-screen shortcuts** (long-press the icon; Android / desktop only): **Record**, and **Type a thought** (`/?mode=text`), which opens straight into text mode. iOS supports neither shortcuts nor share targets for web apps.
